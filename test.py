...
def main(parser_data):
    ...
    # val_dataset = Crack500Instance("/home/leiqin/data_leiqin/datasets/crack500/", dataset="test",transforms=data_transform["val"]
    # val_dataset = VOCInstances(data_root, year="2012", txt_name="val.txt", transforms=data_transform["val"])
    val_dataset = CrackLS315(data_root, dataset="test",transforms=data_transform["val"])
    ...
    with torch.no_grad():
        for image, targets in tqdm(val_dataset_loader, desc="validation..."):
            ...
            if use_best_thr:
                ...
                assert torch.numel(targets[0]['thr']) != 0
                thr_mse_dict[targets[0]['image_id']] = F.mse_loss(pred_thr, targets[0]['thr'])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--k2', default=0, type=int, help='k2 for CCAF ')
    parser.add_argument('--DBT', default=False, type=bool, help = "Switch for DBT branch")
