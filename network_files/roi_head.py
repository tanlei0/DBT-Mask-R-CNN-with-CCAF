...
def thr_loss(thr_pred, thr_gt, pos_matched_idxs):
    for i, proposal in enumerate(pos_matched_idxs):
        repeat_count = proposal.shape
        thr_gt[i] = thr_gt[i].repeat(repeat_count)
    thr_gt = torch.cat(thr_gt, dim = 0)[:,None]
    loss = F.mse_loss(thr_pred, thr_gt)
...
class RoIHeads(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.use_best_thr = use_best_thr
        self.best_thr_head = best_thr_head
        self.best_thr_predictor = best_thr_predictor
    ...
    def has_best_thr(self):
        if not self.use_best_thr:
            return False
        if self.best_thr_head is None:
            return False
        if self.best_thr_predictor is None:
            return False
        return True
    ...
    def forward(self,
        ...
        if self.has_mask():
            ...
            if self.has_best_thr():
                thr_features = self.best_thr_head(mask_features)
                thr_preds = self.best_thr_predictor(thr_features)
            ...
            if self.training:
                ...
                if self.has_best_thr():
                    thr_gt = [t["thr"] for t in targets]
                    best_thr_loss = thr_loss(thr_preds, thr_gt, pos_matched_idxs)
                    loss_thr = {"loss_thr": best_thr_loss}
                    losses.update(loss_thr)
            else:
                ...
                if self.has_best_thr():
                    for thr_pred, r in zip(thr_preds,result):
                        r["thr"] = thr_pred
