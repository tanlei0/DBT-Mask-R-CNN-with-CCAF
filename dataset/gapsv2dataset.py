...
class gapsV2Dataset(data.Dataset):
    ...
    def __init__(self, root, dataset="train", transforms=None, best_thr_file=None):
        super(gapsV2Dataset, self).__init__()
    ...
    def parse_targets(self,
        ...
        if self.use_best_thr:
            best_thr = self.best_thr_df[self.best_thr_df.image == img_id]["thr"]
            target["thr"] = torch.Tensor(best_thr.values)
            if torch.numel(target["thr"]) == 0:
                print("error id", img_id)
            assert torch.numel(target["thr"]) != 0
